...
class CMComparison:
    ...
    def enhanced_fresh_top100(self):
        ...
        df = pd.concat(
            [self.df_fresh, self.delta_ranking(), self.pct_change_market_cap()],
            axis=1,
            sort=False,
            join="outer",
        )
        ...
        if self.usd_percent_change() is not None:
            df = pd.concat(
                [df, self.usd_percent_change(), self.btc_percent_change()],
                axis=1,
                sort=False,
                join="outer",
            )
    ...
    def delta_ranking(self):
        ...
        for i in self.df_old.index:
            if i in self.df_fresh.index:
                index.append(i)
                delta_rank.append(
                    self.df_old["rank"].loc[i] - self.df_fresh["rank"].loc[i]
                )
        delta_df = pd.DataFrame(
            data={"delta_rank": delta_rank}, index=index, dtype=np.int64
        )
    ...
    def pct_change_market_cap(self):
        ...
        for i in self.df_old.index:
            if i in self.df_fresh.index:
                index.append(i)
                pct_change_market_cap.append(
                    (
                        self.df_fresh["usd_market_cap"].loc[i]
                        - self.df_old["usd_market_cap"].loc[i]
                    )
                    * 100
                    / self.df_old["usd_market_cap"].loc[i]
                )
        pct_df = pd.DataFrame(
            data={"pct_change_market_cap": pct_change_market_cap}, index=index
        )
    ...
    def usd_percent_change(self):
        ...
        year_old, month_old, day_old = map(
            int, re.findall(r"\d{4}-\d{2}-\d{2}", self.old_cmc_data)[0].split("-")
        )
        year_fresh, month_fresh, day_fresh = map(
            int, re.findall(r"\d{4}-\d{2}-\d{2}", self.fresh_cmc_data)[0].split("-")
        )
        ...
        if delta.days in {1, 7}:
            print(
                "Information already in the columns:\n \
            ...
            for i in self.df_old.index:
                if i in self.df_fresh.index:
                    index.append(i)
                    usd_percent_change.append(
                        (
                            self.df_fresh["usd_price"].loc[i]
                            - self.df_old["usd_price"].loc[i]
                        )
                        * 100
                        / self.df_old["usd_price"].loc[i]
                    )
            usd_pct_df = pd.DataFrame(
                data={
                    "usd_percent_change_" + str(delta.days) + "d": usd_percent_change
                },
                index=index,
            )
    ...
    def btc_percent_change(self):
        ...
        if delta.days in {1, 7}:
            ...
            for i in self.df_old.index:
                if i in self.df_fresh.index:
                    index.append(i)
                    btc_percent_change.append(
                        (
                            self.df_fresh["btc_price"].loc[i]
                            - self.df_old["btc_price"].loc[i]
                        )
                        * 100
                        / self.df_old["btc_price"].loc[i]
                    )
            btc_pct_df = pd.DataFrame(
                data={
                    "btc_percent_change_" + str(delta.days) + "d": btc_percent_change
                },
                index=index,
            )
